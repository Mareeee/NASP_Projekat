...
func WriteToFile() {
	...
	fmt.Printf("File contents before writing: %s\n", string(mmapFile))

	// Write data to the file by modifying the mapped memory
	dataToWrite := []byte("Hello, World!")
	copy(mmapFile, dataToWrite)

	// Sync the changes back to the file
	err = mmapFile.Flush()
	if err != nil {
		log.Fatal(err)
	}

	fmt.Printf("File contents after writing: %s\n", string(mmapFile))
}
